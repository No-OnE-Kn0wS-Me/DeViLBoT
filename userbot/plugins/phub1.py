"""
Pornhub downloader by @levin_owner
Syntax: .phd link
"""
import datetime
import asyncio
...
async def _(event):
    ...
    async with event.client.conversation("@ttsavebot") as conv:
          try:
              await conv.send_message("/start")
              oop = await conv.get_response()
              if "language" in oop.text:
                  await borg.send_message(event.chat_id, "**Please go to** @ttsavebot **and select your language**")
              await asyncio.sleep(2)
              ...
              if "Downloading" in response.text:
                  ...
                  await borg.send_file(event.chat_id, video_hehe, caption="`🤤 Video Uploaded by` [@levinowner](https://github.com/Dark-Princ3/X-tra-Telegram)!🤤\n**Title:** `{}`".format(title), thumb=photo_thumb)
              ...
              await event.reply("**Please unblock** @ttsavebot **and try again**")
              return
